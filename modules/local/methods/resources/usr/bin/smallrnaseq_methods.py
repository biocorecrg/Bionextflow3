...
parser.add_argument("-v","--methods_version", help="Path to the methods output file", required=True)
...
with open (args.methods_version, 'r') as file:
...
if methods['reporting']['filter_unique'] == 'YES':
    filter_unique = f"<p>Tags mapping to multiple loci were filtered out</p>"
else:
    filter_unique = ""
...
data: |
  <h4>Methods</h4>
  <p>{template['fastqc']} <a href="{tools['fastqc']['homepage']}">FastQC</a> version {tools['fastqc']['version']}.</p>
  <p>{template['trimgalore']} <a href="{tools['trimgalore']['homepage']}">TrimGalore</a> version {tools['trimgalore']['version']}, employing the parameters {methods['progPars']['trimgalore']}.</p>
  <p>{template['shortstack']} {methods['reporting']['genome_name']} using ShortStack.</p>
  {filter_unique}
  <p>Aligned reads were annotated against miRBase and {methods['reporting']['mirnas_name']} databases.</p>
  <p>Coverage profiles were generated using ShortTracks with the {methods['progPars']['shorttracks']} readgroup options.</p>
