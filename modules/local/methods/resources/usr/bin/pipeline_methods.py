...
parser.add_argument("-c", "--citations", help="Path to a citation.yaml file with custom citations that override the defaults", required=False, default=None)
...
custom_citations = {}
if args.citations:
    with open(args.citations, 'r') as file:
        custom_citations = yaml.safe_load(file) or {}
...
for tool in all_tools_info.keys():
  ...
  if tool in custom_citations:
    citations_info[tool] = custom_citations[tool]
  elif ( "citation" in all_tools_info[tool]):
